...
export function PartnerManagement({ className }: PartnerManagementProps) {
  ...
  const fetchPartners = async () => {
    try {
      ...
      const response = await QuestService.getPartners(page, limit, token);
  ...
  const handleCreatePartner = async () => {
    ...
    try {
      ...
      await QuestService.createPartner(newPartnerName.trim(), newPartnerPhoto || undefined, token);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-sm text-muted-foreground font-mono">LOADING_PARTNERS...</p>
      </div>
    );
  }
      ...
      <Card className="border-2 border-dashed border-red-500/20 bg-gradient-to-br from-red-500/5 to-red-600/5">
        <CardContent className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
          ...
          <div className="text-center space-y-2">
            <h3 className="font-semibold text-red-600 font-mono">ERROR_LOADING_PARTNERS</h3>
  ...
  return (
    <div className={cn("space-y-6", className)}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                <DialogTrigger asChild>
                  <Button 
                    className="font-mono gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700"
                  >
                ...
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle className="font-mono">CREATE_NEW_PARTNER</DialogTitle>
                    <DialogDescription>
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={createLoading || !newPartnerName || !newPartnerName.trim()}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Update partner information. Leave photo empty to keep current photo.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={editLoading || !editPartnerName || !editPartnerName.trim()}
              className="font-mono gap-2"
            >
              {editLoading ? (
                <>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-mono text-red-600">DELETE_PARTNER</DialogTitle>
            <DialogDescription>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  {partnerToDelete.photo ? (
                    <Avatar className="h-12 w-12 border-2 border-red-200">
                      <AvatarImage 
                        src={`${process.env.NEXT_PUBLIC_API_URL || 'https://hedera-quests.com'}/uploads/${partnerToDelete.photo}`}
                  ...
                  <div>
                    <div className="font-medium font-mono text-red-700">{partnerToDelete.name}</div>
                    <div className="text-sm text-red-500 font-mono">ID: {partnerToDelete.id}</div>
                    <div className="text-sm text-red-500 font-mono">Code: {partnerToDelete.referral_code}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2 p-3 rounded border border-amber-200 bg-amber-50">
                ...
                <div className="text-sm text-amber-700">
                  <strong>Warning:</strong> Deleting this partner will permanently remove all associated data and referral tracking.
                </div>
              </div>
            </div>
          )}
                  ...
                  filteredPartners.map((partner) => (
                    ...
                    >
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <div>
                            <div className="font-medium font-mono">{partner.name}</div>
                            <div className="text-sm text-muted-foreground font-mono">ID: {partner.id}</div>
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        {partner.photo ? (
                          <Avatar className="h-10 w-10 border-2 border-dashed border-emerald-500/20">
                            <AvatarImage 
                              src={`${process.env.NEXT_PUBLIC_API_URL || 'https://hedera-quests.com'}/uploads/${partner.photo}`}
                              alt={partner.name}
                            />
                          ...
                          <div className="h-10 w-10 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center bg-gray-50">
                    ...
                    <TableCell colSpan={4} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {searchTerm ? 'Try adjusting your search criteria' : 'Create your first partner to get started'}
                        </p>
                      </div>
                    </TableCell>
                  </TableRow>
